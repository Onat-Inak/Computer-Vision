...
%possible improvement (to be evaluated in depth, threshold could be increased):
is_valid_trans=is_valid_trans&(abs((H(1,1)-H(2,2))/(max(abs(H(1:2,1:2)),[],'all')))<0.4)&(abs((H(1,2)+H(2,1))/(max(abs(H(1:2,1:2)),[],'all')))<0.4); % check (a,a) and (b,-b)
