...
% 3.2.1) Apply Difference Magnitude function regarding superpixels for
    % 2 images :

% 3.2.2) Apply Difference Magnitude function regarding superpixels in a timelapse :

% 3.2.2.1) Apply Difference Magnitude function regarding superpixels in a timelapse with 
        % comparing all the figures with their previous ones:
        
% 3.2.2.2) Apply Difference Magnitude function regarding superpixels in a timelapse with 
        % comparing all the figures with the first image in the corresponding folder:
...
% 3.3.1) Apply Difference Magnitude function regarding superpixels in a timelapse with 
        % comparing all the figures with their previous ones and differ the
        % magnitude of changes between images:
        
% 3.3.2) Apply Difference Magnitude function regarding superpixels in a timelapse with 
        % comparing all the figures with the first image and differ the
        % magnitude of changes between images:
...
threshold_DM_3_2 = 50;
    ...
    apply_3_2_for_2_images = false;
    num_ref_img = 4;
    num_moving_img = 5;
    num_superpixels_two_images = 3000;
    ...
    threshold_SP_val_timelapse_3_2 = 90; % threshold for seperating superpixels, which have more difference per pixel than the threshold value in comparison to the others for timelapse
    
        % parameters for 3.2.2.1 :
        comparison_rg_prev_img_3_2 = false; % compare all the images regarding the previous image in a timelapse plot:
        
        % parameters for 3.2.2.2 :
        comparison_rg_first_img_3_2 = true; % compare all the images regarding the first image in a timelapse plot:
...
seg_flag_3_3 = 0;
plot_images_3_3 = 0;
num_superpixels_timelapse_3_3 = 1000;
threshold_SP_val_timelapse_biggest_3_3 = 30; % threshold (in comparison to the most changed superpixel) for seperating superpixels, which have more difference per pixel than the threshold value in comparison to the others for timelapse
threshold_SP_val_timelapse_intermediate_3_3 = 50;
threshold_SP_val_timelapse_low_3_3 = 70;

        % parameters for 3.3.1 :
        comparison_rg_prev_img_3_3 = false; % compare all the images regarding the previous image in a timelapse plot:
        
        % parameters for 3.3.2 :
        comparison_rg_first_img_3_3 = true; % compare all the images regarding the first image in a timelapse plot:
...
if apply_3_1
    ...
    if comparison_rg_first_img_3_1 
...
if apply_3_2
    % 3.2.1) Apply Difference Magnitude function regarding superpixels for
    % 2 images :
    if apply_3_2_for_2_images
        t_3_2__two_images = tic;
        ref_image = Images_reconstructed_new{num_ref_img};
        moving_image = Images_reconstructed_new{num_moving_img};
        [superpixel_pos, N] = superpixels(moving_image, num_superpixels_two_images);
        change_threshold = threshold_DM_3_2;
        seg_flag = seg_flag_3_2;
        plot_Images = plot_images_3_2;
        [~, Diff_Image_Threshold] = Difference_Magnitude(ref_image, moving_image, change_threshold, plot_Images, seg_flag);
        Diff_Image_Threshold(Diff_Image_Threshold ~= 0) = 255;
    ...
    %         sum(Diff_Image_Threshold(pos_sp_reg_img), 'all')
            if sum(Diff_Image_Threshold(pos_sp_reg_img), 'all') / numel(Diff_Image_Threshold(pos_sp_reg_img)) > threshold_SP_val_2_images_3_2
...
%         overlay = imoverlay(moving_image, region_mask_temp, 'red');
        imshow(imoverlay(overlay, BM, 'cyan'),'InitialMagnification',67)
        duration_3_2_two_images = toc(t_3_2__two_images)
    end
    
    % 3.2.2) Apply Difference Magnitude function regarding superpixels in a timelapse : 
    if apply_3_2_for_timelapse
        % 3.2.2.1) Apply Difference Magnitude function regarding superpixels in a timelapse with 
        % comparing all the figures with their previous ones:
        if comparison_rg_prev_img_3_2
            t_3_2_timelapse = tic;
            fig_for_3_2 = figure(3);
            hold on
            for img_num = 1 : length(Images_reconstructed_new) - 1
                [superpixel_pos, N] = superpixels(Images_reconstructed_new{img_num}, num_superpixels_timelapse);
                change_threshold = threshold_DM_3_2;
                seg_flag = seg_flag_3_2;
                plot_Images = plot_images_3_2;
                ref_image = Images_reconstructed_new{img_num};
                ...
                for sp = 1 : N
                    ...
                    pos_sp_reg_img = (superpixel_pos == mat_temp);
            %         sum(Diff_Image_Threshold(pos_sp_reg_img), 'all')
                    if sum(Diff_Image_Threshold(pos_sp_reg_img), 'all') / numel(Diff_Image_Threshold(pos_sp_reg_img)) > threshold_SP_val_timelapse_3_2
                        logical_region_mask = logical_region_mask + pos_sp_reg_img;
                    end
                end
                logical_region_mask = logical(logical_region_mask);
...
%                 imshow(imoverlay(overlay, BM, 'cyan'),'InitialMagnification',67)
        ...
        % 3.2.2.2) Apply Difference Magnitude function regarding superpixels in a timelapse with 
        % comparing all the figures with the first image in the corresponding folder:
        if comparison_rg_first_img_3_2
            t_3_2_timelapse = tic;
            fig_for_3_2 = figure(3);
            hold on
            for img_num = 2 : length(Images_reconstructed_new)
...
%                 imshow(imoverlay(overlay, BM, 'cyan'),'InitialMagnification',67)
    ...
    if apply_3_3
        ...
        % comparing all the figures with their previous ones and differ the
        % magnitude of changes between images:
        if comparison_rg_prev_img_3_3
            t_3_3_timelapse = tic;
            fig_for_3_3 = figure();
            hold on
            for img_num = 1 : length(Images_reconstructed_new) - 1
                [superpixel_pos, N] = superpixels(Images_reconstructed_new{img_num}, num_superpixels_timelapse_3_3);
                change_threshold = threshold_DM_3_3;
                seg_flag = seg_flag_3_3;
                plot_Images = plot_images_3_3;
                ref_image = Images_reconstructed_new{img_num};
                moving_image = Images_reconstructed_new{img_num + 1};
                [~, Diff_Image_Threshold] = Difference_Magnitude(ref_image, moving_image, change_threshold, plot_Images, seg_flag);
                
                [m, n, l] = size(Images_reconstructed_new{1});
                logical_region_mask_biggest = zeros(m, n); 
                logical_region_mask_intermediate = zeros(m, n);
                logical_region_mask_low = zeros(m, n);
                region_mask_red = zeros(m, n);
                region_mask_blue = zeros(m, n);
                region_mask_green = zeros(m, n);
                region_mask = zeros(m, n, 3);
                region_mask_biggest = zeros(m, n, 3);
                region_mask_intermediate = zeros(m, n, 3);
                region_mask_low = zeros(m, n, 3);
                ...
                th_biggest = biggest_change_mean * (100 - threshold_SP_val_timelapse_biggest_3_3) / 100;
                th_intermediate = biggest_change_mean * (100 - threshold_SP_val_timelapse_intermediate_3_3) / 100;
                th_low = biggest_change_mean * (100 - threshold_SP_val_timelapse_low_3_3) / 100;
                for sp = 1 : N
                    mat_temp = sp * ones(m, n);
                    pos_sp_reg_img = (superpixel_pos == mat_temp);
                    mean_change_in_sp = sum(Diff_Image_Threshold(pos_sp_reg_img), 'all') / numel(Diff_Image_Threshold(pos_sp_reg_img));
                    if mean_change_in_sp > th_biggest
                        logical_region_mask_biggest = logical_region_mask_biggest + pos_sp_reg_img;
                    elseif mean_change_in_sp > th_intermediate
                        logical_region_mask_intermediate = logical_region_mask_intermediate + pos_sp_reg_img;
                    elseif mean_change_in_sp > th_low
                        logical_region_mask_low = logical_region_mask_low + pos_sp_reg_img; 
                    else
                        continue;
                    end       
                end
                % biggest changes -> red
                logical_region_mask_biggest = logical(logical_region_mask_biggest);
                region_mask_red(logical_region_mask_biggest) = 255;
...
%                 region_mask_intermediate(:,:,3) = region_mask_blue;
                ...
                % low changes -> green
                logical_region_mask_low = logical(logical_region_mask_low);
                region_mask_green(logical_region_mask_low) = 255;
...
%                 region_mask = region_mask_biggest + region_mask_intermediate + region_mask_low;
    ...
    %             figure();
    %             imshow(imoverlay(region_mask, BM, 'cyan'),'InitialMagnification',67)
                overlay = imoverlay(Images_reconstructed_new{img_num}, region_mask_red, 'red');
                overlay = imoverlay(Images_reconstructed_new{img_num}, region_mask_blue, 'blue');
                overlay = imoverlay(Images_reconstructed_new{img_num}, region_mask_green, 'green');
                clf
                % show the overlayed image without boundary mask :
                imshow(overlay);
                % show the overlayed image with boundary mask : 
%                 imshow(imoverlay(overlay, BM, 'cyan'),'InitialMagnification',67);
        ...
        % comparing all the figures with the first image and differ the
        % magnitude of changes between images:
        if comparison_rg_first_img_3_3
            t_3_3_timelapse = tic;
            fig_for_3_3 = figure();
            hold on
            for img_num = 2 : length(Images_reconstructed_new)
