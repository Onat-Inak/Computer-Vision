...
W=what;
dir_path = W.path;
dir_list  = dir(fullfile(dir_path, 'Rain*.jpg'));
files = {dir_list.name};
...
H_cell=cell(length(files));
ssd_mat=zeros(length(files));%to save ssd
comp_cell=cell(length(files));%to save diff-images or overlapp
is2DTransform=zeros(length(files)); %Matrix to check if transform is 'in plane' in the image plane

%figure();
for i=1:length(files)
    for j=1:length(files)
        try
        [comp,ssd,H_ran]=reorientate_4pa(imgs{i},imgs{j});%orient imgs(j) into imgs(i)
        comp_cell{i,j}=comp;
        H_cell{i,j}=H_ran;
        ssd_mat(i,j)=ssd;
        is2DTransform(i,j)=(abs((H_ran(3,1))+abs(H_ran(3,2)))/abs(H_ran(3,3)))<0.0005;%(H_ran(3,1) and H_ran(3,2) have to be zero for a 2D in-plane transform in the image plane
%         subplot(length(files),length(files),i+(j-1)*length(files));
%         imshow(comp);%currently only as subplot, if need arises: plot comp(i,j) maually
        catch %not enough points or matches
        H_cell{i,j}=zeros(3);
        ssd_mat(i,j)=inf;
        is2DTransform(i,j)=0;
        end
...
% ssd_mat=ssd_mat';
% is2DTransform=is2DTransform'; %for comparison on subplots

% for i=1:length(files)
%     for j=1:length(files)
%     H_ran=H_cell{i,j};
%     is2DTransform(i,j)=(abs((H_ran(3,1))+abs(H_ran(3,2)))/abs(H_ran(3,3)))<0.0005;
%     end
% end
...
for i=1:length(files)
    for j=1:length(files)
        subplot(length(files),length(files),i+(j-1)*length(files)),
        imshow(comp_cell{i,j});%currently only as subplot, if need arises: plot comp(i,j) maually
    end
end

function [comp,ssd,H_ran]=reorientate_4pa(I1,I2)
%output comp: überlagertes Bild
I1=im2double(I1{1});
if length(size(I1))>2
I1=rgb2gray(I1);
end
...
%calculate ssd on overlapping regions:
%(simlification: image(a,b)=0-> not in overlaping region)

Mask=(reoriented_Im2>0);%Mask only due to reconstructed image, the other image lies in its own image region
comp=abs(I1-reoriented_Im2).*Mask;
ssd=sum(comp.^2,'all','omitnan');
